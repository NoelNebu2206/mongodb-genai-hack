...
async def get_git_data_endpoint(request: Request):
    ...
    logging.info('Documents saved on MongoDB.')
...
async def get_previous_conversation(request: Request):
    ...
    logging.info("Starting the Chat ID retrieval process")
...
async def user(request: Request):
    ...
    data = await request.json()
    email = data.get('email')
    logging.info("Starting the user verification process")
    chats = mongoDbClient.user.remote(email=email)
    logging.info('User Document Created')
